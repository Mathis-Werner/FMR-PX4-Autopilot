...
void *baseRateTask(void *arg)
{
  runModel = (UrbanConderController2_M->getErrorStatus() == (NULL));
  while (runModel) {
    px4_sem_wait(&baserateTaskSem);
    UrbanConderController2_step();

    // Get model outputs here
    stopRequested = !((UrbanConderController2_M->getErrorStatus() == (NULL)));
  }
...
{
  UNUSED(sig);
  UrbanConderController2_M->setErrorStatus("stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  // Terminate model
  UrbanConderController2_terminate();
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  UrbanConderController2_M->setErrorStatus(0);

  // Initialize model
  UrbanConderController2_initialize();
